...
| **Agent HTTP server** | Port `8001` on the head node | **Required.** The post-evaluation harness connects to `localhost:8001` on the head node. Any other port will cause you to fail. |

> **Added 2026-07-31, from a fresh portal export — not in the original copy of this file.** The
> submission-guide.md example endpoint (`agent :5000`) is a generic placeholder only. The actual
> required port for your own agent server (the one exposing `/health` and `/query`) is **8001 on the
> head node** — this is a hard requirement, not a default. Confirm with an organizer which of the two
> cluster nodes is the "head node" before deploying, since the brief only distinguishes
> "brain/agent node" and "fine-tuning/model node" elsewhere.
