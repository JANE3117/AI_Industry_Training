...
_tool_llm_api_key = os.getenv(
    "TOOL_LLM_API_KEY", os.getenv("OPENAI_API_KEY", "sk-123456")
)
_tool_llm_base_url = os.getenv(
    "TOOL_LLM_BASE_URL",
    os.getenv("OPENAI_BASE_URL", "http://ai-gateway:4000"),
)
_tool_llm_model_name = os.getenv(
    "TOOL_LLM_MODEL", os.getenv("MODEL_NAME", "llama-distributed")
)
# See _IS_AZURE in agent.py -- same extra header, applied to this tool's own
# (possibly different) model/gateway.
_tool_llm_is_azure = (
    "cognitive.microsoft.com" in _tool_llm_base_url
    or "openai.azure.com" in _tool_llm_base_url
)

music_expert_model = ChatOpenAI(
    openai_api_key=_tool_llm_api_key,
    openai_api_base=_tool_llm_base_url,
    model_name=_tool_llm_model_name,
    temperature=0.01,
    http_async_client=_tool_llm_http_async_client,
    default_headers={"api-key": _tool_llm_api_key} if _tool_llm_is_azure else None,
)
