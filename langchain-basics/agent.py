...
# Azure OpenAI's v1 (OpenAI-compatible) surface still requires an api-key
# header alongside the Authorization header ChatOpenAI already sends -- the
# extra header is a no-op against the internal gateway, so it's always safe
# to include.
_IS_AZURE = "cognitive.microsoft.com" in OPENAI_BASE_URL or "openai.azure.com" in OPENAI_BASE_URL
...
model = ChatOpenAI(
    ...
    default_headers={"api-key": OPENAI_API_KEY} if _IS_AZURE else None,
)
